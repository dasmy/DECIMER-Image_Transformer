...
class TpuBatchNormalization(tf.keras.layers.BatchNormalization):
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        ...
        if num_shards_per_group > 1:
            logging.info(
                "TpuBatchNormalization with num_shards_per_group %d",
...
def normalization(
    ...
    return BatchNormalization(axis=axis, momentum=momentum, epsilon=epsilon, name=name)
...
def num_params_flops(readable_format=True):
    ...
    flops = tf.compat.v1.profiler.profile(
        tf.compat.v1.get_default_graph(), options=options
    ).total_float_ops
...
def float16_scope():
    ...
    with tf.compat.v1.variable_scope("", custom_getter=_custom_getter) as varscope:
        yield varscope
...
def get_ckpt_var_map(
    ckpt_path, ckpt_scope="", var_scope="", skip_mismatch=None, init_ema=True
):
    ...
    for v in model_vars:
        ...
        if v.shape != cv_shape:
            if skip_mismatch:
                logging.info(
                    "skip %s (%s vs %s) -- shape mismatch", v_name, v.shape, cv_shape
                )
                continue
